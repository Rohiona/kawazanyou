...
export function getChatGPTUser(
  ...
  if (!email) return getLocalPreviewUser(env);
...
function getLocalPreviewUser(env: AuthEnv): ChatGPTUser | null {
  if (env.KAWAZANYOU_LOCAL_AUTH_ENABLED !== "true") return null;

  const email = env.KAWAZANYOU_LOCAL_USER_EMAIL?.trim();
  if (!email) return null;
  const fullName = env.KAWAZANYOU_LOCAL_USER_FULL_NAME?.trim() || null;
